...
const dbExceptions =
    level(require('os').homedir() + '/.ethfaucetssl/exceptions');
const greylistduration = 0;

var faucet_keystore = JSON.stringify(require('./wallet.json'));
//var faucet_keystore = JSON.stringify(require(
 //   './UTC--2018-01-18T01-05-42.637024000Z--1f7402f55e142820ea3812106d0657103fc1709e.json'));
...
var web3Provider = new HookedWeb3Provider(
    {host: config.web3.host, transaction_signer: keystore});
...
//     {
//       password: config.walletpwd,
//       seedPhrase:
//           'question eager diagram hill fall marriage hub nuclear disease
//           manage bread awkward',
//       hdPathString: 'm/44\'/60\'/0\'/0'
//     },
//     function(err, ks) {
//       ks.keyFromPassword(config.walletpwd, function(err, pwDerivedKey) {
//         var keystore = new lightwallet.keystore.deserialize(faucet_keystore);
//         console.log('connecting to ETH node: ', config.web3.host);
...
function getFaucetBalance(denomination) {
  console.log('balance ' + web3.eth.getBalance(account));
  return parseFloat(web3.fromWei(
      web3.eth.getBalance(account).then(toNumber()), denomination || 'ether'));
}
...
app.get('/blacklist/:address', function(req, res) {
  ...
  if (isAddress(address)) {
    setException(address, 'blacklist').then(() => {
      res.status(200).json({msg: 'address added to blacklist'});
    });
  } else {
    return res.status(400).json({message: 'the address is invalid'});
  }
});
...
function getQueue() {
  ...
  return new Promise((resolve, reject) => {
    var stream = dbQueue.createReadStream({keys: true, values: true})
                     .on('data',
                         item => {
                           q.push(item);
                         })
...
function exceptionsLength() {
  return new Promise((resolve, reject) => {
    var lengths = {};
    dbExceptions.createReadStream({keys: true, values: true})
        .on('data',
            function(item) {
...
function enqueueRequest(address) {
  return new Promise((resolve, reject) => {
    ...
    dbQueue.put(
        key, JSON.stringify({created: Date.now(), address: address}),
        function(err) {
          if (err) {
            return reject(err);
          }
...
function setException(address, reason) {
  return new Promise((resolve, reject) => {
    dbExceptions.put(
        address,
        JSON.stringify({created: Date.now(), reason: reason, address: address}),
        function(err) {
          if (err) {
            return reject(err);
          }
...
function cleanupException() {
  var stream = dbExceptions.createReadStream({keys: true, values: true})
                   .on('data', item => {
...
app.get('/donate/:address', function(req, res) {
  ...
  if (isAddress(address)) {
    ...
    Promise.all([getException(address), getException(ip)])
        .then(([addressException, ipException]) => {
          ...
          if (exception) {
            ...
            if (exception.reason === 'blacklist') {
              ...
              return res.status(403).json(
                  {address: address, message: 'you are blacklisted'});
            }
          } else {
              ...
              if (canDonate) {
                ...
                queueLength().then(length => {
                  if (length < config.queuesize) {
                    ...
                    return res.status(403).json({msg: 'queue is full'});
                  }
                });
              }
            });
...
function donate(to, cb) {
  web3.eth.getGasPrice(function(err, result) {
    ...
    var options =
        {from: account, to: to, value: amount, gas: 314150, gasPrice: gasPrice};
    console.log(options);
